  ...
  TemplateLoader.prototype.createTemplateFromString = function(directiveType, encapsulation, template, templateSourceUrl, styles, styleUrls) {
    ...
    allStyles = allStyles.map(function(style) {
      ...
      styleWithImports.styleUrls.forEach(function(styleUrl) {
        return allStyleUrls.push(styleUrl);
      });
      return styleWithImports.style;
    });
    var allResolvedStyles = allStyles.map(function(style) {
      return _this._styleUrlResolver.resolveUrls(style, templateSourceUrl);
    });
    var allStyleAbsUrls = allStyleUrls.map(function(styleUrl) {
      return _this._urlResolver.resolve(templateSourceUrl, styleUrl);
    });
...
var TemplatePreparseVisitor = (function() {
  ...
  TemplatePreparseVisitor.prototype.visitAttr = function(ast) {
    return ast;
  };
  TemplatePreparseVisitor.prototype.visitText = function(ast) {
    return ast;
  };
